							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
											<td><?php echo ($v["id"]); ?></td>
											<td><a target="_blank" href="<?php echo U('inMember',array('u'=>$v['member']));?>"><?php echo ($v["member"]); ?></a></td>
											...
											<td>
												...
												<?php if($v["status"] == 0): ?><a onclick="if(confirm('一旦操作，不可修改，确认通过审核吗?')==false)return false;"  href="<?php echo U(GROUP_NAME .'/Member/sqtg',array('id'=>$v['id']));?>">通过</a> |
												<a onclick="if(confirm('拒绝后，会自动封禁申请人的账号，确认拒绝吗?')==false)return false;"  href="<?php echo U(GROUP_NAME .'/Member/sqjj',array('id'=>$v['id']));?>">拒绝</a><?php endif; ?>
