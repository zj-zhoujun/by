		...
		<div id="page-content" class="clearfix">
			<div class="page-header position-relative">
				<div style="text-align:left;width:100%;">
					<a type="button" href="<?php echo U(GROUP_NAME.'/Shop/addhuodong');?>" class="btn btn-info btn-small no-border"> <i class="icon-plus icon-white"></i>添加活动</a>
				</div>
			</div>
			...
			<div class="row-fluid">
				...
				<form id="table-searchbar" method="POST" class="form-inline well well-small">
					<div class="row-fluid">
						&nbsp;&nbsp;开始日期
						<input type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="input-small" value="<?php echo $_POST['be_logtime'];?>" name="be_logtime">
						&nbsp;&nbsp;截止日期
						<input type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="input-small" value="<?php echo $_POST['en_logtime'];?>" name="en_logtime">
				...
				<div class="row-fluid">
					<table id="table_report" class="table table-striped table-bordered table-hover">
						...
						<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
								...
								<td><?php echo ($v["zszc"]); ?></td>
								<td><?php echo ($v["kjname"]); ?></td>
								<td><?php echo ($v["kj_num"]); ?></td>
								...
								<td>
										...
										<a href="<?php echo U(GROUP_NAME .'/Shop/edithd',array('id'=>$v['id']));?>"><font color="red">修改</font></a>
									|
										<a href="<?php echo U(GROUP_NAME .'/Shop/delhd',array('id'=>$v['id']));?>" onclick="if(!confirm('删除后不可恢复，确认要删除吗？')){return false;}"><font color="#4169e1">删除</font></a>
    ...
    $(function() {
        ...
        $('#clear-log').click(function(){
            bootbox.confirm("确认要清空日志吗?", function(result) {
                if(result) {
                    var url = '<?php echo U(GROUP_NAME .'/Log/deleteLog');?>';
                    window.location = url;
                }
            });
        });
