<?php  
	...
	Class InfoAction extends CommonAction{
        ...
        public function updatenew(){
            $ann = M('updatenew')->where(array('id'=>1))->find();
            $this->assign('ann',$ann);
            $this->display();
        }

        //修改更新公告处理
        public function editupdatenewHandle(){
            $_POST['addtime'] = time();
            M('updatenew')->where(array('id'=>1))->data($_POST)->save();
            //添加日志操作
            $desc = '修改更新公告';
            write_log(session('username'),'admin',$desc);

            $this->success('修改成功',U(GROUP_NAME.'/Info/updatenew'));
        }
